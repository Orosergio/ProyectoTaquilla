            ...
            if (lblApellidoCliente.Text != "")
            {
                ...
                if (chkPago.Checked)
                {
                    ...
                    try
                    {
                        ...
                        string insertarEncabezado = "INSERT INTO FACTURAENCABEZADO (IDFACTURAENCABEZADO,SERIE,FECHA,NITCLIENTE,TOTAL,DESCUENTO,IDPROYECCIONPELICULA,PUNTOS,ESTATUS) " +
                            "VALUES(" + codigoEnc + ",'" + "BGCLC-" + codigoEnc + "',?," + txtNit.Text.ToString() + "," + decimal.Parse(lblCantTotal.Text.ToString()) + "," + decimal.Parse(lblCantDescuento.Text.ToString()) + "," + idProyeccionPelicula + ","+lblCantPuntos.Text.ToString()+"," + 1 + ")";
